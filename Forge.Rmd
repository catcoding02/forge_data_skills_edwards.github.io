...
library(tidycensus)
library(spData) #gives polygons for US States in data frame called us_states.
```

Now, we need to read the 2019 CDC Abortion Surveillance Data into our session. 
```{r}
abortions_usa <- read.csv("abortions-distributed-by-state-2019.csv", skip = 1)
```
...
```{r}
abortions_usa_1 <- abortions_usa[53, 2:53] #Select relevant rows and columns
abortions_usa_1 <- rename(abortions_usa_1, Number_of_Abortions = State.Area) #Rename column to accurate, descriptive name
abortions_usa_2 <- pivot_longer(abortions_usa_1, cols = 1:52, names_to = "State", values_to = "Number_of_Abortions_In_State") #Pivot data table longer for more sensible information presentation and to prep for palette creation, which creates palettes based on a column, not a row, of values
abortions_usa_2 <- abortions_usa_2[2:52, ] #Select for relevant rows
abortions_usa_2$Number_of_Abortions_In_State <- as.numeric(abortions_usa_2$Number_of_Abortions_In_State) #Transform Number_of_Abortions_In_State column from "character" class to "numeric" so that the colorNumeric palette can determine the range of the numbers and assign to color palette
```
...
```{r}
abortions_usa_2$State <- gsub("\\..$", "", abortions_usa_2$State) #Removed excess periods from certain states' names

abortions_usa_2$State <- gsub("\\.$", "", abortions_usa_2$State) #Removed excess periods from certain states' names

abortions_usa_2$State <- gsub("[...]", "_", abortions_usa_2$State) #Replaced '..." with "_" in certain states

abortions_usa_2$State[33] <- "New_York" #renamed New York City and State entry to New York

us_states$NAME <- gsub(" ", "_", us_states$NAME) #replace every " " with a "_" in us_states data frame to match with the name format for states in abortions_usa_2
```
...
census_abortions_joined <- census_states |> 
  ...
  inner_join(abortions_usa_2, 
             by = c("NAME" = "State")) #Join relevant columns of census_states to abortions_usa_2
census_abortions_joined <- census_abortions_joined |> 
  mutate(abortions_per_capita = 
           Number_of_Abortions_In_State / summary_value) #Create new column in census_abortions_joined that calculates abortions per capita for each state
...
```{r}
red_blue_states <- read.csv("Red_Blue_States.csv") 
